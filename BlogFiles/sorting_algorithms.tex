...
\script{populate_page.js}

\section{A Thorough Analysis of Known Sorting Algorithms}
This blog contains several sorting algorithms, along with their pseudocode, an implementation to test an algorithm and
view how it works, the worst- average- and best-case time complexity and their spatial complexity.\\
Immediately after this introduction, you'll be met with a page where you can compare two sorting algorithms, visually.
\\\\
\subsubsection{Terms}
A sorting algorithm is said to be\\
- \underline{in-place} if it's spatial complexity is $\Theta(1)$,\\
- \underline{stable} if it maintains the relative order of elements with equal keys\footnote{
    When sorting integers this is irrelevant, but more often than not sorting algorithms sort objects based on
    'keys' and it makes sense to care about stability. Suppose you have a log that displays error messages and the
    timestamp of the error. A sorting algorithm sorts these messages in order of recency and 
    unless your sorting algorithm is <u>stable</u> these error messages will jump around, 
    switching places, making the messages impossible to read.
}\\
...
the original array was, this algorithm will sort the array in $\frac{n(n-1)}{2}-1=
\Theta(n^2)$ steps 
...
\[
    (n-1)+(n-2)+\cdots+3+2=\sum_{i=2}^{n-1} i=\sum_{i=1}^{n-1} i-1=\frac{n(n-1)}{2}-1=\Theta (n^2)
\]
...
In the figure above we can see visually that this algorithm is $\Theta(n^2)$. For each value of
$i$ and $j$, we see how much work is perfromed.\\
...
lower bound, since theres also work in the green squares that we dont account for, thus $f(n)=\Omega(n^2)$. 
Now the entire square of size $(n-1)^2$ is an upper bound for the amount of work performed (combining the green and red squares) 
since not all the work in this large square is 1, thus we also have an upper bound $f(n)=\mathcal O (n^2)$. 
Since the upper and lower bounds are the same we have our precise estimate $f(n)=\Theta(n^2)$.
\\\\
Now in terms of spatial complexity we create only variables <code>minIndex</code>, $i$, $j$
and a temporary variable for swapping elements, so in total 4 variables, which
...
<h4>Time- and spatial complexity</h4>
Bubble sort only uses extra storage when swapping elements, thus the spatial complexity is $\Theta(1)$.\\
The time complexity for bubble sort is in the worst case and average case $\Theta(n^2)$, but $\Theta(n)$ in the 
best case.
\\\\
...
<h4>Time- and spatial complexity</h4>
\textsc{InsertionSort}, like all previous algorithms uses constant space to swap elements (and keep track of variables $i$ and $j$), 
thus the spatial complexity is $\Theta(1)$.\\
The best case for this algorithm is $\Theta(n)$ and the average and worst case are both $\Theta(n^2)$.
\\\\
Here's why:\\
Let $\mathcal S_i$ be the number of iterations of the inner while loop, then the time complexity of \textsc{InsertionSort}
is $\Theta\left(\sum(1+\mathcal S_i)\right)$ and $\mathcal S_i$ is exactly the number of times where $\A[j - 1]>\A[j]$ for 
any $1\leq j\leq i$ (note that $\mathcal S_i$ is not affected by rearranging the earlier elements). 
\\\\
If $\mathcal S_i$ is the number of times the while loop runs in the $i$'th iteration of the for loop, then the
run time for \textsc{InsertionSort} is $=\displaystyle\sum_{i=1}^{|\A|-1}(1+\mathcal S_i)$. The average value of 
$1+\mathcal S_i$ is equal to
\begin{equation*}
    \begin{split}
...
has to move all the way to the front of the array) and the average value of $S_i\sim\frac{n}{2}$ making the worst case run time
$\Theta(n^2)$.\\
In the average case the $i$'th element has $i/2$ elements before it, that is larger than itself. Thus 
$S_i\sim\frac{n}{4}$ and the average time complexity is $\Theta(n^2)$ aswell.\\
...
##Merge##(~A~, ~l~, ~m~, ~r~)
    ...
    while L < |leftHalf| and R < |rightHalf|
        i = l + L + R

        //Since the halves are sorted, the smallest element in each array is 
        //the first element - This is what makes merge O(n)
    ...
    //If there are no elements left in the right half, copy the remaining 
    ...
    //If there are no elements left in the left half, copy the remaining 
    //elements of the right half
    while R < |rightHalf|
        i = l + L + R
        ~A~[i] = rightHalf[R]
        R++
...
\textsc{Quicksort} is a very efficient sorting algorithm, often performing better than any other (non-hybrid)
sorting algorithm. A major reason this is often faster than sorts like \textsc{MergeSort} is the fact that it
can easily be implemented as an \underline{in-place} sorting algorithm. 
\\\\
Unlike previous algorithms this algorithm is not stable.
